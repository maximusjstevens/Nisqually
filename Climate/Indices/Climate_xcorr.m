...
for ii=1:length(thenames)
   ...
   MXSD.(thenames{ii})=xcorr(IN_scaled,MXSD_scaled,'biased');
   TSNW.(thenames{ii})=xcorr(IN_scaled,TSNW_scaled,'biased');
   TPCP.(thenames{ii})=xcorr(IN_scaled,TPCP_scaled,'biased');
   MNTM.(thenames{ii})=xcorr(IN_scaled,MNTM_scaled,'biased');
   
%    MXSD.(thenames{ii})=xcorr(Indata_d(U1),Pmxsd(U1),'biased');
%    TSNW.(thenames{ii})=xcorr(Indata_d(U2),Ptsnw(U2));
%    TPCP.(thenames{ii})=xcorr(Indata_d(U3),Ptpcp(U3),'biased');
%    MNTM.(thenames{ii})=xcorr(Indata_d(U4),Pmntm(U4),'biased');
